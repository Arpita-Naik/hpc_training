...
class SlurmInstaller:
    ...
    def get_latest_version(self):
        print("Fetching latest Slurm version from SchedMD...")
        ...
        try:
            ...
            if matches:
                ...
                print("Latest version detected:", latest)
                return latest

        except Exception as e:
            print("Failed to fetch latest version:", e)

        print("Using default version:", self.DEFAULT_VERSION)
    ...
    def install_dependencies(self, pkg_manager):
        ...
        if pkg_manager == "apt":
            ...
            packages = [
                ...
                "mariadb-devel", "json-c-devel",
                "hwloc-devel", "libbpf-devel",
                "dbus-devel", "systemd-devel",
                "numactl-devel", "pkgconfig",
                "readline-devel",
                "bison", "flex",
                "mariadb-server", "curl", "wget"
            ]

            self.run(["sudo", pkg_manager, "install", "-y"] + packages)
    ...
    def configure_path(self):
        ...
        path_export = f"{self.PREFIX}/bin:{self.PREFIX}/sbin"

        with open("/tmp/slurm.sh", "w") as f:
            f.write(f'export PATH="{path_export}:$PATH"\n')

        self.run(["sudo", "mv", "/tmp/slurm.sh", "/etc/profile.d/slurm.sh"])
        self.run(["sudo", "chmod", "755", "/etc/profile.d/slurm.sh"])

        bashrc = os.path.expanduser("~/.bashrc")
        line = f'export PATH="{path_export}:$PATH"'
        ...
        os.environ["PATH"] = f"{path_export}:" + os.environ["PATH"]


    def create_command_symlinks(self):

        print("==== Creating Slurm command symlinks ====")
...
"""
    ...
    def verify(self):
        ...
        if result.returncode == 0:
            ...
            print("✔ Slurm Installed Successfully")
            print(result.stdout)

        else:

            print(result.stderr)
            raise Exception("Slurm verification failed")
